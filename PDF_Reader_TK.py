...
# Lấy tổng số trang file pdf
pages = pdfReader.numPages
print(pages)

# Thông tin giọng đọc với voice 1 là nam và voice 0 là giọng nữ
bot = pyttsx3.init()
voices = bot.getProperty('voices')
bot.setProperty('voices', voices[0].id)
"""
# Chọn số trang cần đọc
layout = [[sg.Text('Enter a Number')],
          [sg.Input()],
          [sg.OK()] ]

window = sg.Window('Nhập số trang cần đọc', layout)

event, StartPage = window.read()
print (StartPage)
window.close()

PageIn= int(StartPage)
#sg.Popup(event, values[0])
"""
# Đọc file pdf với số trang được định chọn
